...
var addSyncingCard = function(streamEvent, user, repos){
	return q.Promise(function(resolve, reject){
		...
		logger.silly(user.username, 'adding sync card', [date, streamEvent.appDbId]);

		var card = {};
		card.userId = user._id;	
		card.type = "datasyncing";
		card.thumbnailMedia = 'chart.html';
		card.appDbId = streamEvent.appDbId;
		card.cardDate = date;
		card.generatedDate = new Date().toISOString();
		card.published = true;
		...
		repos.cards.insert(card, function(error, response){
			if(error){
				logger.error(user.username, 'error inserting sync card', [error, streamEvent.appDbId]);
				reject(error);			
			}
			else
			{
				logger.debug(user.username, 'sync card inserted', [streamEvent.appDbId, response.result]);
				resolve();
			}
		});
	});
...
var removeSyncingCard = function(streamEvent, user, repos){
	return q.Promise(function(resolve, reject){
		...
		logger.debug(user.username, 'removing syncing card, condition', JSON.stringify(condition));
...
var addCardsGeneratingCard = function(streamEvent, user, repos){
	return q.Promise(function(resolve, reject){
		var date = streamEvent.dateTime.substring(0, 10);
		logger.silly(user.username, 'adding cards generating card', [date, streamEvent.appDbId]);
		var card = {};
		card.userId = user._id;	
		...
		repos.cards.insert(card, function(error, response){
			if(error){
				logger.error(user.username, 'error inserting cards generating card', [error, streamEvent.appDbId]);
				reject(error);			
			}
			else
			{
				logger.debug(user.username, 'cards generating card inserted', [streamEvent.appDbId, response.result]);
				resolve();
			}
		});
	});
...
var removeCardsGeneratingCard = function(streamEvent, user, repos){
	return q.Promise(function(resolve, reject){
		var date = streamEvent.dateTime.substring(0, 10);
		logger.silly(user.username, 'removing cards generating card', [date, streamEvent.appDbId]);
		var condition = {
			userId: user._id,
			cardDate: date,
